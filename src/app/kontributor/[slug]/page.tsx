...
import { fetchContributors, fetchPublishedArticles } from '@/lib/server-api'
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const contributor = contributors.find((c: any) => c.slug === params.slug)
...
export default async function KontributorDetailPage({ params }: Props) {
  const [contributors, allArticles] = await Promise.all([
    fetchContributors(),
  ...
  const contributorArticles = allArticles
    .map(mapArticle)
    .filter((a: any) => a.author.slug === params.slug)
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {contributorArticles.map((article: any) => (
